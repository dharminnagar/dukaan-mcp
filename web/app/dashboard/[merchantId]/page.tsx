...
import type { Decision, ReasonCode } from "../../../lib/dashboard-queries";
...
export default async function DashboardPage({
  ...
  const [revenue, spend, decisions] = await Promise.all([
    loadRevenueSummary(merchantId),
        ...
        <div className="rounded-md border border-[var(--color-border)] bg-white px-5 py-4">
          ...
          {spend === null ? (
            ...
            <>
              ...
              <p className="mt-1 text-sm text-[var(--color-muted)]">
                bound by: {spend.effective_cap.bound_by}
              </p>
            </>
          )}
        </div>
