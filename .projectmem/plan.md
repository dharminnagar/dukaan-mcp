...
- [x] DUK-28 DONE 25 Aug. Audit sessions are derived per (merchant, agent) per 30-min window when the client sends no header, so any third-party client groups correctly. Verified end to end: 3 calls, no header, 1 session. Stateful transport rejected (would hold a revoked token for the session's life). Limitation for the README: sessions are derived, not negotiated at the protocol level.
