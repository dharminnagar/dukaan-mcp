...
const CSV_A = await Bun.file(
  `${REPO_ROOT}/fixtures/demo-merchant-a.csv`
).text();
const CSV_B = await Bun.file(
  `${REPO_ROOT}/fixtures/demo-merchant-b.csv`
).text();
...
function findProduct(
  products: typeof productsA,
  id: string
): (typeof productsA)[number] {
...
describe("DUK-11 demo catalog design (fixtures only, no DB)", () => {
  ...
  test("merchant A (kirana) is visibly low-price / high-stock; merchant B (electronics) is visibly high-price / low-stock", () => {
    ...
    const meanStockA =
      productsA.reduce((sum, p) => sum + p.stock, 0) / productsA.length;
    const meanStockB =
      productsB.reduce((sum, p) => sum + p.stock, 0) / productsB.length;
  ...
  test("policies differ on all four fields: spend_cap, approval_threshold, category_allowlist, window_seconds", () => {
    ...
    expect(policyA.approval_threshold_paise).not.toBe(
      policyB.approval_threshold_paise
    );
  ...
  test("catalogs include categories outside each merchant's own allowlist, so CATEGORY_NOT_ALLOWED is reachable", () => {
    const outsideA = productsA.filter(
      (p) => !policyA.category_allowlist.includes(p.category)
    );
    const outsideB = productsB.filter(
      (p) => !policyB.category_allowlist.includes(p.category)
    );
  ...
  describe("the five adversarial attack classes", () => {
    test("1. budget-split evasion: many small A orders of Chana Dal accumulate past the spend cap without any single order tripping the approval threshold", () => {
      ...
      expect(ordersNeeded * orderValue).toBeGreaterThan(
        policyA.spend_cap_paise
      );
    ...
    test("4. threshold straddling: an item priced between B's and A's approval thresholds escalates on B but not on A", () => {
      ...
      const escalatesOnB =
        orderValue > policyB.approval_threshold_paise &&
        orderValue <= policyB.spend_cap_paise;
      const escalatesOnA =
        orderValue > policyA.approval_threshold_paise &&
        orderValue <= policyA.spend_cap_paise;
...
describe("DUK-11 seed-demo execution (DB-backed)", () => {
  test("seeding twice is idempotent: same merchant/policy/product rows, and the price-distribution fixture matches the catalog", async () => {
    ...
    const distribution: Array<{
      ...
      `${REPO_ROOT}/fixtures/demo-price-distribution.json`
    ).json();
