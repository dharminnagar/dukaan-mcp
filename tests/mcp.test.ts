...
describe("list_products / get_product tenancy", () => {
  ...
  test("no tool declares a merchant_id argument, on either tool, for either merchant", async () => {
    ...
    expect(
      Object.keys(
        (listProducts?.inputSchema as InputSchemaShape).properties ?? {}
      )
    ).toEqual([]);
    ...
    expect(
      Object.keys(
        (getProduct?.inputSchema as InputSchemaShape).properties ?? {}
      )
    ).toEqual(["id"]);
...
describe("checkout", () => {
  test("a stale asserted price is rejected with STALE_CATALOG, isError true, and no order row", async () => {
    ...
    const result = await client.callTool({
      name: "checkout",
      arguments: {
        items: [{ item_id: "sku-a1", quantity: 1, asserted_price_paise: 1 }],
      },
    });
    await client.close();
    ...
    const orders = await query<{ id: string }>(
      "SELECT id FROM orders WHERE merchant_id = $1",
      [MERCHANT_A]
    );
  ...
  test("a valid order is created, audited exactly once as allow, survives being read by a fresh repo, and stays invisible to another merchant", async () => {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        items: [
          { item_id: "sku-a1", quantity: 1, asserted_price_paise: 19900 },
        ],
      },
    });
    await client.close();
    ...
    if (!resolvedA.ok)
      throw new Error("resolveTenant unexpectedly failed for tokenA");
    ...
    await clientB.close();
    const { order: crossOrder } = JSON.parse(textOf(crossResult)) as {
      order: unknown;
    };
  ...
  test("an order above the approval threshold is escalated, never reaches Razorpay, and carries the gate-minted order id", async () => {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        items: [
          { item_id: "sku-a2", quantity: 3, asserted_price_paise: 49900 },
        ],
      },
    });
    await client.close();
  ...
  test("an order blocked by the spend cap never reaches Razorpay and writes no order row", async () => {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        items: [
          { item_id: "sku-c1", quantity: 1, asserted_price_paise: 20000 },
        ],
      },
    });
    await client.close();
  ...
  test("a Razorpay-side failure on the allow path fails the order and audits RAZORPAY_ERROR, without throwing", async () => {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        items: [
          { item_id: "sku-b1", quantity: 1, asserted_price_paise: 29900 },
        ],
      },
    });
    await client.close();
    ...
    const razorpayFailureRows = failureAudit.filter(
      (r) => r.reason_code === "RAZORPAY_ERROR"
    );
...
describe("checkout under concurrency (DUK-29)", () => {
  test("N parallel checkouts that each fit under the cap cannot collectively beat it", async () => {
    ...
    for (let i = 0; i < N; i++) {
      concurrencyFake.enqueue({
        ok: true,
        razorpay_order_id: `order_mcp29_${i}`,
      });
    }
    ...
    try {
      const clients = await Promise.all(
        Array.from({ length: N }, () => connect(tokenD))
      );
              ...
              items: [
                {
                  item_id: "sku-d1",
                  quantity: 1,
                  asserted_price_paise: AMOUNT_PAISE,
                },
              ],
            },
          })
