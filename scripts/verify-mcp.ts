...
async function main(): Promise<void> {
  ...
  const listA = await clientA.callTool({
    name: "list_products",
    arguments: {},
  });
  console.log(
  ...
  const listB = await clientB.callTool({
    name: "list_products",
    arguments: {},
  });
  console.log(
  ...
  console.log(
    ...
    ownProduct.content?.[0]?.type === "text"
      ? ownProduct.content[0].text
      : ownProduct
  );
  ...
  console.log(
    ...
    crossTenant.content?.[0]?.type === "text"
      ? crossTenant.content[0].text
      : crossTenant
  );
