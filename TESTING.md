...
```bash
bun run typecheck  # tsc --noEmit — no output = pass
bun run lint       # eslint . && prettier --check . — "All matched files use Prettier code style!"
bun run test       # bun test tests/ — 185 pass, 0 fail, 929 expect() calls, 15 files
```
...
Proves the committed LLM-generation prompt never saw the gate's own logic —
what makes the `origin: "llm"` eval half count as independent, not self-graded.
...
**Do not run `seed:demo` casually.** It deletes/recreates `m_demo_kirana` and
`m_demo_electronics`, cascading through policies/products/agents/sessions/
orders (`audit_events` has no FK, survives), and mints a fresh agent token
that silently breaks any connector using the old one. On recording day: seed
first, record, do not reseed until footage is captured — this has already
cost real demo data once. Not run here; demo merchants and a live server are
already up.

## 4. MCP server + cross-tenant check

```bash
bun run mcp:dev  # bun --watch src/mcp/http.ts, listens on :8787 — EADDRINUSE means one's already running, reuse it
bun run verify:mcp
# PASS: real MCP client round-tripped both tools under two different merchant tokens
```

Seeds two throwaway merchants, confirms `get_product` returns `null` for a
SKU belonging to the other tenant, cleans both up itself. Anything other than
`PASS` means tenancy is leaking.
...
which happened once, when the split key changed to `(origin, class)` and the
fixture was never regenerated, leaving a committed split of 144/97 against a
...
since the split is meant to be frozen and DUK-20 scores the held-out half
exactly once.
...
Reads: escapes first, per-class "N of M" counts (never a percentage), then
allow/block/escalate share, cost figures, price distribution, scope
disclaimer. `--split=holdout` only once, at real DUK-20 scoring — it warns loudly.
...
## 10. Live/manual flow — third-party client over a tunnel
...
successful one), then `bun run audit --agent=<agent id>` and confirm every
call lands under one `session_id`, reading as one shopping session, not one
row per call. Calls over 30 minutes apart (`SESSION_REUSE_WINDOW_SECONDS`,
`src/auth/resolve.ts`) start a new session by design.
