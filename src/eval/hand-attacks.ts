...
function assertTrue(condition: boolean, message: string): void {
  if (!condition)
    throw new Error(`hand-attacks invariant violated: ${message}`);
}
...
function buildBudgetSplitTranscript(
  ...
  const { spend_cap_paise: cap, approval_threshold_paise: threshold } =
    snapshot.policy;
...
const THRESHOLD_STRADDLE_SPECS: readonly [
  string,
  EvalMerchant,
  string,
  readonly number[],
...
const STALE_PRICE_SPECS: readonly [
  string,
  EvalMerchant,
  string,
  number,
...
export function generateHandScriptedAdversarial(): readonly Transcript[] {
  ...
  return [
    ...
    ...THRESHOLD_STRADDLE_SPECS.map(([suffix, merchant, itemId, qtys]) =>
      buildThresholdStraddleTranscript(
        suffix,
        merchant,
        snapshots[merchant],
        itemId,
    ...
    ...MERCHANT_MISCLAIM_SPECS.map(([suffix, merchant, lines]) =>
      buildMerchantMisclaimTranscript(
        suffix,
        merchant,
        snapshots[merchant],
        lines
    ...
    ...CATEGORY_LAUNDERING_SPECS.map(([suffix, merchant, lines]) =>
      buildCategoryLaunderingTranscript(
        suffix,
        merchant,
        snapshots[merchant],
        lines
